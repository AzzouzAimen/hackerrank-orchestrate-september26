...
The stable implementation and its engine/boundary tests now live in
[`code/buy_or_wait/`](../code/README.md). Local `evidence.py`, `finance.py`, `plans.py`
and old test modules are compatibility imports only. The canonical schema is
[`code/buy_or_wait/evidence.schema.json`](../code/buy_or_wait/evidence.schema.json).
References, reports, representative runs and extraction experiments remain here.
...
python -m unittest prototype.test_boundary -v
...
`evidence.py` defines seven discriminated fact types (a type-specific payload selected
by fact_type). `evidence.schema.json` exports that contract. Every nested object forbids
...
scores reviewed references, and writes `prototype/extraction_artifacts/`. It never replaces
`prototype/facts/` or writes trusted recommendations. See
