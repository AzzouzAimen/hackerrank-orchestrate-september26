...
class LedgerTests(unittest.TestCase):
    ...
    def test_structured_scheduled_salary_counts_but_other_credit_does_not(self):
        salary=event('salary',status='scheduled',direction='credit',event_type='income',category='salary',amount='100')
        bonus=event('bonus',status='scheduled',direction='credit',event_type='income',category='bonus',amount='500')
        s=project(state([salary,bonus]),{})
        self.assertEqual([(flow.source,flow.amount) for flow in s.flows],[('salary',D('100'))])
